...
Установка включает **автозапуск**: `docker` и юнит `std-cards.service` добавляются в
автозагрузку, при включении сервера стек поднимается сам (`systemctl status std-cards`).

### Защита сервера

`install.sh` и `update.sh` вызывают `setup-security.sh` — он идемпотентно настраивает:

| Что | Как |
|---|---|
| Нестандартный порт SSH | `SSH_PORT` из `.env` (по умолчанию 54368) |
| Запрет входа под root | `PermitRootLogin no` — только если есть другой пользователь с sudo и ssh-ключом |
| Файрвол | `ufw`: входящие запрещены, открыты SSH, 80, 443 |
| Бан за перебор и сканирование | `fail2ban`: джейлы `sshd`, `caddy-auth`, `caddy-scan`, `recidive` |
| Лимит запросов | модуль `rate_limit` в Caddy: вход 10/мин, API 300/мин, публичные карточки 120/мин |
| Блок ботов | типовые пути сканеров (`/wp-login.php`, `/.env`, …) и User-Agent'ы → 404 |
| Заголовки | HSTS, X-Content-Type-Options, X-Frame-Options, Referrer-Policy |
| TLS и редирект | Let's Encrypt + автообновление + HTTP→HTTPS — Caddy делает сам |
| Внутренние сервисы | postgres/nats/minio наружу не публикуются, только docker-сеть |

Порт 22 **остаётся открытым**, пока вы не проверите вход на новый порт:

```bash
ssh -p 54368 <user>@<домен>      # из ВТОРОЙ сессии, старую не закрывать
sudo ./setup-security.sh --drop-22
sudo ./verify-security.sh        # 12 проверок по пунктам ТЗ, входит в postcheck.sh
```

На стендах, которые делят хост с другими сервисами, поставьте в `.env`
`SECURITY_SETUP=off` — тогда SSH, ufw и fail2ban не трогаются.

### Бэкапы на отдельный диск

По умолчанию бэкапы пишутся в `./backups` рядом с установкой (на системный диск).
Чтобы складывать их на отдельный HDD:

```bash
sudo ./setup-backup-disk.sh /dev/sdX   # покажет содержимое диска и спросит подтверждение
```

Скрипт показывает, что на диске лежит, форматирует его в ext4 **только после явного
подтверждения**, монтирует в `/mnt/std-cards-backup` (запись в `/etc/fstab`, переживает
перезагрузку) и прописывает `BACKUP_DIR` в `.env`. Системный диск форматировать откажется.
После него — `docker compose up -d` (перечитает `BACKUP_DIR`).
...
- **03:00 ежедневно** — `pg_dump | gzip` в `$BACKUP_DIR` (по умолчанию `./backups/`), ротация 30 дней.

### Импорт фото из Excel

В шаблоне импорта есть колонка **«Ссылка на фото»**. Поддерживаются публичные ссылки
Яндекс.Диска (`https://disk.yandex.ru/i/...`) и прямые ссылки на изображение
(jpeg/png/webp, до 15 МБ). Фото скачивается и ставится на карточку автоматически.
Если ссылка битая или файл не скачался — карточка всё равно создаётся, а строка
попадает в отчёт об ошибках импорта.
...
**Обновление до актуальной версии** (одной командой — pull, бэкап, пересборка, перезапуск, проверка):

```bash
sudo ./update.sh
```

Для offline-установки: привезти новый бандл, заменить `images/*.tar` и запустить `sudo ./update.sh`.
Важно: не разворачивайте копию в другом каталоге на том же сервере — docker compose
считает это тем же проектом и пересоздаст контейнеры с чужим `.env`.
